...
from src.pydts.model_selection import PenaltyGridSearch
...
class TestPenaltyGridSearch(unittest.TestCase):
    ...
    def test_get_mixed_two_stages_fitter(self):
        pgs = PenaltyGridSearch()
        train_df, test_df = train_test_split(self.patients_df.drop(['C', 'T'], axis=1), random_state=0)
        pgs.evaluate(train_df=train_df,
                     test_df=test_df,
                     l1_ratio=1,
                     metrics=[],
                     penalizers=[0.005, 0.02],
                     seed=0)
        mixed_two_stages = pgs.get_mixed_two_stages_fitter([0.005, 0.02])
    ...
    def test_assertion_get_mixed_two_stages_fitter_not_included(self):
        with self.assertRaises(AssertionError):
            ...
            mixed_two_stages = pgs.get_mixed_two_stages_fitter([0.1, 0.02])

    def test_assertion_get_mixed_two_stages_fitter_empty(self):
        with self.assertRaises(AssertionError):
            pgs = PenaltyGridSearch()
            mixed_two_stages = pgs.get_mixed_two_stages_fitter([0.001, 0.02])

    def test_evaluate(self):
        pgs = PenaltyGridSearch()
        train_df, test_df = train_test_split(self.patients_df.drop(['C', 'T'], axis=1), random_state=0)
        idx_max = pgs.evaluate(train_df=train_df, test_df=test_df, l1_ratio=1,
    ...
    def test_convert_results_dict_to_df(self):
        ...
        idx_max = pgs.evaluate(train_df=train_df, test_df=test_df, l1_ratio=1,
                               ...
                               seed=0)
        pgs.convert_results_dict_to_df(pgs.global_bs)
